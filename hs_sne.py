...
# The implementation of HS-SNE is modified based on the code of t-SNE implementation in scikit-learn.
# Below is the original meta information of the t-SNE implementation in scikit-learn.
...
def binary_search_perplexity_vonmises(distances, perplexity, tolerance=1e-3):
    ps = np.zeros(distances.shape)
    kappas = np.zeros(distances.shape[0])
    desired_entropy = np.log(perplexity)
    for i in range(len(distances)):
        entropy_diff = tolerance
        kappa_min = 0
        kappa_max = np.finfo(np.double).max
        kappa = 1
        while (np.abs(entropy_diff) >= tolerance) and (kappa_max - kappa_min > 1e-4):
            p = np.exp(kappa * (np.cos(distances[i]) - 1))
            if distances.shape[0] == distances.shape[1]:
                p[i] = 0
            if p.sum() == 0:
                entropy_diff = -tolerance-1
            else:
                # Do not need to normalize p, since entropy(p) == entropy(p*2)
                entropy_diff = entropy(p) - desired_entropy
            if (entropy_diff > 0):
                kappa_min = kappa
                kappa = min(kappa * 2, kappa_max)
            else:
                kappa_max = kappa
                kappa = kappa_min/2 + kappa_max/2
        p = p/p.sum()
        ps[i] = p
        kappas[i] = kappa
    return ps, kappas
...
def _joint_probabilities(distances, desired_perplexity, verbose):
    ...
    conditional_P, kappas = binary_search_perplexity_vonmises(distances, desired_perplexity)
...
def _joint_probabilities_nn(distances, desired_perplexity, verbose):
    ...
    conditional_P, kappas = binary_search_perplexity_vonmises(distances_data, desired_perplexity)
    ...
    if verbose >= 2:
        duration = time() - t0
        print("[HS-SNE] Computed conditional probabilities in {:.3f}s".format(duration))
    return P
    ...
    for i in range(it, n_iter):
        ...
        if check_convergence:
            ...
            if error < best_error:
                ...
                if verbose >= 2:
                    print(
                        "[HS-SNE] Iteration %d: did not make any progress "
            ...
            if grad_norm <= min_grad_norm:
                if verbose >= 2:
                    print(
                        "[HS-SNE] Iteration %d: gradient norm %f. Finished."
                        % (i + 1, grad_norm)
                    )
                break
...
class HSSNE(BaseEstimator):
    ...
    _parameter_constraints: dict = {
        ...
        "metric": [StrOptions(set(_VALID_METRICS) | {"precomputed", "geodesic"}), callable],
    ...
    def _fit(self, X, skip_num_points=0):
        ...
        if self.method == "exact":
            ...
            if self.metric == "precomputed":
                ...
                if self.verbose:
                    print("[HS-SNE] Computing pairwise distances...")
                ...
                if self.metric == "euclidean":
                    ...
                    distances = pairwise_distances(
                        X, metric="cosine", n_jobs=self.n_jobs)
                    distances = np.arccos(1-distances)
                else:
            ...
            if self.verbose:
                print("[HS-SNE] Computing {} nearest neighbors...".format(n_neighbors))
            ...
            if self.verbose:
                print(
                    "[HS-SNE] Indexed {} samples in {:.3f}s...".format(
                        n_samples, duration
                    )
                )
            ...
            if self.verbose:
                print(
                    "[HS-SNE] Computed neighbors for {} samples in {:.3f}s...".format(
                        n_samples, duration
                    )
                )
    ...
    def _tsne(
        ...
        if self.verbose:
            print(
                "[HS-SNE] KL divergence after %d iterations with early exaggeration: %f"
                % (it + 1, kl_divergence)
            )
        ...
        if self.verbose:
            print(
                "[HS-SNE] KL divergence after %d iterations: %f"
                % (it + 1, kl_divergence)
            )
